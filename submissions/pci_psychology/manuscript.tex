...
\urlstyle{same}
\hypersetup{
  pdftitle={Czech Adaptation and Initial Validation of a Historical Perspective Taking Instrument: Ideological Fairness Evidence},
  pdfauthor={Juda Kaleta},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Czech Adaptation and Initial Validation of a Historical Perspective Taking Instrument: Ideological Fairness Evidence}
\author{Juda Kaleta}
\date{2026}
...
\textbf{Keywords:} historical perspective taking, test adaptation, measurement invariance, differential item functioning, construct-irrelevant variance, test fairness
...
The present study reports the first Czech adaptation and psychometric evaluation of the Hartmann and Hasselhorn HPT instrument. Two considerations motivate this work. First, no validated Czech-language HPT instrument currently exists, despite growing interest in historical thinking assessment within Czech education reform \citep{Cinatl_et_al2021Historicka,Ripka_et_al2024Epistemic}. The Czech adaptation addresses this gap by providing a candidate instrument for further Czech validation and future cross-national equivalence studies.
...
\citep[pp.~54, 57]{RVPZV2023}. It does not prescribe a fixed grade-level
...
\citep{Kaleta2025Sticking}. Participating teachers reported that the
...
they relate to the factor; \citealp{Flora2004}). Omega values were
...
\textbf{Missing data and score construction.} Revision-reporting
...
Table 3 summarizes reliability for the primary study scores. Across HPT subscales, raw Cronbach's \(\alpha\) ranged from .46 (POP) to .64 (CONT), polychoric \(\alpha\) ranged from .52 to .69, and McDonald's \(\omega\) ranged from .54 to .70. CONT demonstrated the strongest reliability, consistent with its highest average factor loadings. Full subscale and predictor estimates are in Table S3.
...
These values are modest by conventional standards and constrain
individual-level interpretation. The three-item design contributes to
this imprecision, but brevity alone does not establish adequate
reliability. With \emph{k} = 3 and inter-item correlations of .20--.35,
the Spearman-Brown formula projects \(\alpha\) = .64--.79 for six-item
subscales; because this assumes tau-equivalence, the projections are
only approximate. McDonald's \(\omega\) (.53--.70) is more appropriate
given unequal loadings (Table 2). The primary six-item composite had
\(\omega\) = .66 and the nine-item total had \(\omega\) = .70. For the
predictors, FR-LF had \(\alpha\) = .67 and \(\omega\) = .71, KSA-3 had
\(\alpha\) = .72 and \(\omega\) = .75, and the combined ideology
composite had estimated reliability .80, 95\% bootstrap CI [.74, .84].
...
correction, six of nine items (POP1, POP2, POP3, ROA1, ROA3, CONT3) had
raw \emph{p}-values below .10. P-values do not identify direction, and
this pattern could reflect chance, multidimensionality, local
dependence, anchor contamination, or weak DIF. The all-others-as-anchor
strategy without iterative purification is vulnerable when multiple
anchor items contain DIF \citep{Woods2009}. The result should therefore
be interpreted as ``no DIF detected with this sample and procedure,''
not ``no DIF present.''
...
Consequently, no configural, metric, or scalar invariance level can be
...
\emph{n} = 125 (High), with 42 middle-group cases excluded. The lower
...
constraints were added (configural CFI = .960, RMSEA = .050, SRMR = .070;
...
qualified by inadmissible smaller-group solutions. This is the first psychometric evaluation of a Czech adaptation of the Hartmann and Hasselhorn HPT instrument and the first investigation of ideological fairness for any HPT measure. The Czech adaptation showed modest subscale reliability, a positive relationship with scenario-relevant historical knowledge, and sensitivity to school-level variation. Political ideology, authoritarianism, and social desirability were unrelated to HPT, although weak reliability constrains the latter null associations.
...
\item \emph{Single country and scenario.} The study uses one national
  context and one historical scenario, so scenario-specific wording,
  ...
  rather than merely to another sample using this one scenario.
...
\item \emph{No response process evidence.} The critical unanswered
  ...
  A targeted follow-up should use retrospective cognitive interviews
