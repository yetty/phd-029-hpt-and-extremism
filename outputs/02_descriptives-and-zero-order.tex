...
\CommentTok{\# Ensure factors as in codebook}
...
\CommentTok{\# HPT (1{-}4): subscores and total (average so all stay on 1{-}4)}
\NormalTok{hpt\_pop\_items  }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"POP1"}\NormalTok{,}\StringTok{"POP2"}\NormalTok{,}\StringTok{"POP3"}\NormalTok{)}
...
\NormalTok{dat }\OtherTok{\textless{}{-}}\NormalTok{ all\_data }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{HPT\_POP  =} \FunctionTok{scale\_mean}\NormalTok{(., hpt\_pop\_items,  }\AttributeTok{min\_n =} \DecValTok{2}\NormalTok{),}
    ...
    \AttributeTok{HPT\_TOTAL =} \FunctionTok{scale\_mean}\NormalTok{(., }\FunctionTok{c}\NormalTok{(hpt\_pop\_items, hpt\_roa\_items, hpt\_cont\_items), }\AttributeTok{min\_n =} \DecValTok{5}\NormalTok{),}
    \AttributeTok{KN\_TOTAL  =} \FunctionTok{rowSums}\NormalTok{(}\FunctionTok{select}\NormalTok{(., }\FunctionTok{all\_of}\NormalTok{(kn\_items)), }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{),}
...
\begin{quote}
\textbf{How to read:} • \textbf{HPT}: 1-4, higher = better fit of
reasoning to historical context/agent constraints (POP/ROA/CONT per
instrument). • \textbf{Knowledge (KN\_TOTAL)}: 0-6 correct. •
\textbf{FR-LF mini (FRLF\_MINI; plus RD, NS)}: 1-5, higher = stronger
endorsement (e.g., leader/one-party; NS relativization). • \textbf{KSA-3
(KSA\_TOTAL)}: 1-5, higher = stronger authoritarianism. •
\textbf{SDR\_TOTAL}: 1-5, higher = stronger social desirability response
tendency. Variable names and coding follow the project codebook.
\end{quote}
...
\FunctionTok{kable}\NormalTok{(}\FunctionTok{data.frame}\NormalTok{(}
  ...
  \AttributeTok{classes =}\NormalTok{ dplyr}\SpecialCharTok{::}\FunctionTok{n\_distinct}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{class\_label),}
  \AttributeTok{schools =}\NormalTok{ dplyr}\SpecialCharTok{::}\FunctionTok{n\_distinct}\NormalTok{(dat}\SpecialCharTok{$}\NormalTok{school\_id)}
\NormalTok{), }\AttributeTok{caption =} \StringTok{"Sample counters (after loading and scoring)."}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{desc\_vars }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"HPT\_POP"}\NormalTok{,}\StringTok{"HPT\_ROA"}\NormalTok{,}\StringTok{"HPT\_CONT"}\NormalTok{,}\StringTok{"HPT\_TOTAL"}\NormalTok{,}
...
\midrule
\cellcolor{gray!10}{HPT\_POP} & \cellcolor{gray!10}{160} & \cellcolor{gray!10}{1.97} & \cellcolor{gray!10}{0.67} & \cellcolor{gray!10}{1} & \cellcolor{gray!10}{3.67}\\
HPT\_ROA & 160 & 2.83 & 0.68 & 1 & 4.00\\
\cellcolor{gray!10}{HPT\_CONT} & \cellcolor{gray!10}{160} & \cellcolor{gray!10}{2.74} & \cellcolor{gray!10}{0.74} & \cellcolor{gray!10}{1} & \cellcolor{gray!10}{4.00}\\
HPT\_TOTAL & 160 & 2.51 & 0.39 & 1 & 3.33\\
\cellcolor{gray!10}{KN\_TOTAL} & \cellcolor{gray!10}{164} & \cellcolor{gray!10}{3.35} & \cellcolor{gray!10}{1.62} & \cellcolor{gray!10}{0} & \cellcolor{gray!10}{6.00}\\
\addlinespace
FRLF\_RD & 159 & 2.46 & 0.92 & 1 & 5.00\\
\cellcolor{gray!10}{FRLF\_NS} & \cellcolor{gray!10}{160} & \cellcolor{gray!10}{2.35} & \cellcolor{gray!10}{0.89} & \cellcolor{gray!10}{1} & \cellcolor{gray!10}{5.00}\\
FRLF\_MINI & 158 & 2.41 & 0.76 & 1 & 4.83\\
\cellcolor{gray!10}{KSA\_TOTAL} & \cellcolor{gray!10}{158} & \cellcolor{gray!10}{2.88} & \cellcolor{gray!10}{0.67} & \cellcolor{gray!10}{1} & \cellcolor{gray!10}{4.78}\\
SDR\_TOTAL & 158 & 3.06 & 0.64 & 1 & 4.40\\
\bottomrule
\end{longtable}
...
\toprule
 & Var1 & HPT\_POP & HPT\_ROA & HPT\_CONT & HPT\_TOTAL & KN\_TOTAL & FRLF\_RD & FRLF\_NS & FRLF\_MINI & KSA\_TOTAL & SDR\_TOTAL\\
\midrule
HPT\_POP & HPT\_POP & 1.00 & -0.17 & -0.36 & 0.24 & -0.32 & 0.08 & 0.07 & 0.09 & 0.15 & 0.06\\
HPT\_ROA & HPT\_ROA & -0.17 & 1.00 & 0.42 & 0.76 & 0.29 & -0.06 & -0.06 & -0.07 & 0.06 & 0.03\\
HPT\_CONT & HPT\_CONT & -0.36 & 0.42 & 1.00 & 0.67 & 0.25 & -0.08 & 0.00 & -0.05 & 0.05 & 0.06\\
HPT\_TOTAL & HPT\_TOTAL & 0.24 & 0.76 & 0.67 & 1.00 & 0.15 & -0.04 & 0.00 & -0.02 & 0.15 & 0.10\\
KN\_TOTAL & KN\_TOTAL & -0.32 & 0.29 & 0.25 & 0.15 & 1.00 & -0.12 & -0.20 & -0.19 & -0.02 & 0.03\\
\addlinespace
FRLF\_RD & FRLF\_RD & 0.08 & -0.06 & -0.08 & -0.04 & -0.12 & 1.00 & 0.40 & 0.84 & 0.46 & -0.04\\
FRLF\_NS & FRLF\_NS & 0.07 & -0.06 & 0.00 & 0.00 & -0.20 & 0.40 & 1.00 & 0.83 & 0.35 & -0.22\\
FRLF\_MINI & FRLF\_MINI & 0.09 & -0.07 & -0.05 & -0.02 & -0.19 & 0.84 & 0.83 & 1.00 & 0.49 & -0.15\\
KSA\_TOTAL & KSA\_TOTAL & 0.15 & 0.06 & 0.05 & 0.15 & -0.02 & 0.46 & 0.35 & 0.49 & 1.00 & -0.14\\
SDR\_TOTAL & SDR\_TOTAL & 0.06 & 0.03 & 0.06 & 0.10 & 0.03 & -0.04 & -0.22 & -0.15 & -0.14 & 1.00\\
\bottomrule
\end{longtable}
...
positively with \textbf{HPT\_CONT}/\textbf{HPT\_TOTAL}, this suggests
possible ideological alignment inflating contextualization---an effect
to test with controls in models (knowledge, SDR) and with item-level
checks later.
\end{quote}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# {-}{-}{-} Robust ICC helpers (replace previous version) {-}{-}{-}}

\CommentTok{\# Safely pull ICC\_adjusted (fallback to ICC or first numeric) for different object types}
\NormalTok{get\_icc\_value }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(ic) \{}
  \CommentTok{\# ic can be data.frame/tibble, list, or numeric}
  ...
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{is.data.frame}\NormalTok{(ic)) \{}
    \CommentTok{\# new performance::icc often returns a data.frame with ICC and ICC\_adjusted}
    ...
    \CommentTok{\# else, first numeric column}
...
\NormalTok{  \}}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{is.list}\NormalTok{(ic)) \{}
    ...
    \CommentTok{\# first numeric element}
...
\NormalTok{fit\_icc }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(v) \{}
  \CommentTok{\# guard: need some variance and at least 2 clusters}
...
\NormalTok{  f\_cls }\OtherTok{\textless{}{-}} \FunctionTok{as.formula}\NormalTok{(}\FunctionTok{paste0}\NormalTok{(v, }\StringTok{" \textasciitilde{} 1 + (1|school\_id/class\_label)"}\NormalTok{))}
...
\NormalTok{extract\_icc }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(fm) \{}
  ...
  \CommentTok{\# Try ICC; handle different output types}
...
\NormalTok{  icc\_val }\OtherTok{\textless{}{-}} \FunctionTok{get\_icc\_value}\NormalTok{(ic)}
  \CommentTok{\# Sample size and cluster count}
\NormalTok{  N }\OtherTok{\textless{}{-}} \FunctionTok{tryCatch}\NormalTok{(}\FunctionTok{nobs}\NormalTok{(fm), }\AttributeTok{error =} \ControlFlowTok{function}\NormalTok{(e) }\ConstantTok{NA\_integer\_}\NormalTok{)}
  \CommentTok{\# Number of levels of the *first* grouping factor in the model}
...
\NormalTok{targets }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"HPT\_TOTAL"}\NormalTok{,}\StringTok{"HPT\_POP"}\NormalTok{,}\StringTok{"HPT\_ROA"}\NormalTok{,}\StringTok{"HPT\_CONT"}\NormalTok{,}
...
\NormalTok{  mods }\OtherTok{\textless{}{-}} \FunctionTok{fit\_icc}\NormalTok{(sc)}
  \CommentTok{\# extract once per model}
...
\midrule
\cellcolor{gray!10}{HPT\_TOTAL} & \cellcolor{gray!10}{0.038} & \cellcolor{gray!10}{160} & \cellcolor{gray!10}{13} & \cellcolor{gray!10}{0.036} & \cellcolor{gray!10}{160} & \cellcolor{gray!10}{7}\\
HPT\_POP & NA & 160 & 13 & 0.025 & 160 & 7\\
\cellcolor{gray!10}{HPT\_ROA} & \cellcolor{gray!10}{0.043} & \cellcolor{gray!10}{160} & \cellcolor{gray!10}{13} & \cellcolor{gray!10}{0.042} & \cellcolor{gray!10}{160} & \cellcolor{gray!10}{7}\\
HPT\_CONT & 0.056 & 160 & 13 & 0.046 & 160 & 7\\
\cellcolor{gray!10}{FRLF\_MINI} & \cellcolor{gray!10}{0.077} & \cellcolor{gray!10}{158} & \cellcolor{gray!10}{13} & \cellcolor{gray!10}{0.057} & \cellcolor{gray!10}{158} & \cellcolor{gray!10}{7}\\
\addlinespace
KSA\_TOTAL & 0.148 & 158 & 13 & 0.114 & 158 & 7\\
\cellcolor{gray!10}{KN\_TOTAL} & \cellcolor{gray!10}{NA} & \cellcolor{gray!10}{164} & \cellcolor{gray!10}{13} & \cellcolor{gray!10}{0.022} & \cellcolor{gray!10}{164} & \cellcolor{gray!10}{7}\\
SDR\_TOTAL & NA & 158 & 13 & NA & 158 & 7\\
\bottomrule
\end{longtable}
...
\NormalTok{p1 }\OtherTok{\textless{}{-}}\NormalTok{ dat }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(school\_id, class\_label) }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{pivot\_longer}\NormalTok{(}\FunctionTok{c}\NormalTok{(HPT\_TOTAL, FRLF\_MINI), }\AttributeTok{names\_to=}\StringTok{"scale"}\NormalTok{, }\AttributeTok{values\_to=}\StringTok{"mean"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Class"}\NormalTok{, }\AttributeTok{y =} \StringTok{"Class mean"}\NormalTok{, }\AttributeTok{title =} \StringTok{"Class means within schools (HPT vs FR{-}LF)"}\NormalTok{)}
