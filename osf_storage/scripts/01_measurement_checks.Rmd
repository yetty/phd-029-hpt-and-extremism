...
dat <- dat %>%
  ...
  mutate(
    HPT_POP_raw = scale_mean(., POP_rev_items, 2),
...
if (has_fr) {
  ...
  scales_list$FR_RD     <- scale_mean(fr_df, paste0("RD", 1:3), 2)
  scales_list$FR_NS     <- scale_mean(fr_df, paste0("NS", 1:3), 2)
  scales_list$FR_total  <- scale_mean(fr_df, fr_cols, 4)
}

if (has_ksa) {
  ksa_df <- dat[keep, ksa_cols]
  scales_list$KSA_A     <- scale_mean(ksa_df, paste0("A", 1:3), 2)
  scales_list$KSA_U     <- scale_mean(ksa_df, paste0("U", 1:3), 2)
  scales_list$KSA_K     <- scale_mean(ksa_df, paste0("K", 1:3), 2)
  scales_list$KSA_total <- scale_mean(ksa_df, ksa_cols, 7)
}

if (has_sdr) {
  sdr_df <- dat[keep, sdr_cols]
  scales_list$SDR_total <- scale_mean(sdr_df, sdr_cols, 4)
}
