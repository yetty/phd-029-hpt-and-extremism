...
dat <- dat %>%
  mutate(
    hpt_pop_rev = scale_mean(., paste0(pop_items, "_rev"), 2),
    hpt_cont    = scale_mean(., cont_items, 2),
    hpt_roa     = scale_mean(., roa_items, 2),
...
dat <- dat %>%
  mutate(
    frlf_rd  = scale_mean(., rd_items, 2),
    frlf_ns  = scale_mean(., ns_items, 2),
    frlf_tot = scale_mean(., c(rd_items, ns_items), 4)
  )
...
dat <- dat %>%
  mutate(
    ksa3_a   = scale_mean(., a_items, 2),
    ksa3_u   = scale_mean(., u_items, 2),
    ksa3_k   = scale_mean(., k_items, 2),
    ksa3_tot = scale_mean(., ksa_items, 7)
  )
...
dat <- dat %>%
  mutate(
    sdr5_tot = scale_mean(., sdr_items, 4)
  )
```
