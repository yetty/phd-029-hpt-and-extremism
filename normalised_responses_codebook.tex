...
\texttt{normalised\_responses.RDS} (R native) and 
\texttt{normalised\_responses.xlsx} (Excel). 
...
{\small
\begin{longtable}{p{3cm} p{1.5cm} p{5.5cm} p{4cm}}
...
\texttt{school\_level}  & factor  & School level (vocational, gymnasium, etc.) & categorical \\
...
\texttt{class\_id}      & factor  & Unique class identifier (school + class) & e.g., S01.1, S01.2, S02.9A \\
...
The HPT instrument assesses how well explanations align with Hannes's perspective in the 1930 vignette. Items are organised into three reasoning modes: POP (populist), ROA (role-of-agent), and CONT (context). 

\textbf{Note on scoring:} Raw POP items represent \textit{presentism} (historically flawed reasoning). These are reversed so that higher scores always indicate \textit{better} historical perspective-taking.

{\small
\begin{longtable}{p{3.2cm} p{1.5cm} p{5.3cm} p{4cm}}
\caption{Codebook --- HPT Items \& Composites}\\
\toprule
\textbf{Variable} & \textbf{Type} & \textbf{Description} & \textbf{Coding}\\
\midrule
\endfirsthead
...
\multicolumn{4}{l}{\textbf{Item-level Data}} \\
\texttt{POP1-POP3}   & integer & Raw populist items (Presentism). & 1--4 (High = \textbf{Worse} HPT) \\
\texttt{POP1\_rev\dots} & integer & \textbf{Reverse-coded} POP items. & 1--4 (High = \textbf{Better} HPT) \\
\texttt{ROA1-ROA3}   & integer & Role-of-agent items. & 1--4 (High = Better HPT) \\
\texttt{CONT1-CONT3} & integer & Contextual reasoning items. & 1--4 (High = Better HPT) \\

\midrule
\multicolumn{4}{l}{\textbf{Aggregates \& Composites}} \\
\texttt{HPT\_POP\_raw} & numeric & Mean of raw POP items (Presentism scale). & 1--4 \\
\texttt{HPT\_POP\_rev} & numeric & Mean of reversed POP items. & 1--4 \\
\texttt{HPT\_CONT}     & numeric & Mean of CONT items. & 1--4 \\
\texttt{HPT\_ROA}      & numeric & Mean of ROA items. & 1--4 \\
\texttt{HPT\_CTX6}     & numeric & \textbf{Primary Composite}: Mean of \texttt{HPT\_POP\_rev} and \texttt{HPT\_CONT}. & 1--4 (Standard HPT metric) \\
\texttt{HPT\_TOT9}     & numeric & \textbf{Total Score}: Mean of all 3 subscales (POP rev, CONT, ROA). & 1--4 \\
\end{longtable}
}
